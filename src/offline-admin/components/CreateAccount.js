...
const CreateWorkerAccount = () => {
  ...
  const onSubmit = (data) => {
    ...
    if (!editData) {
      ...
      updatePassword({ id: editData._id, password });
    }
  };
  ...
  return (
    <div className="flex flex-col items-center py-10 bg-gray-50 min-h-screen">
      <div className="w-full max-w-md bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold mb-6">{editData ? "Change Worker Password" : "Create Worker Login"}</h2>
      ...
      {latestWorker && (
        <div className="mt-6 w-full max-w-md bg-white p-4 rounded shadow">
          <h3 className="font-semibold text-lg mb-2">🎉 Worker Created Preview</h3>
          ...
          <p><strong>Created At:</strong> {new Date(latestWorker.createdAt).toLocaleString()}</p>
        </div>
      )}
    </div>
  );
